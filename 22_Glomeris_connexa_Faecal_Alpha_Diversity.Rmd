...
## **Read the data and create Glomeris (faecal and faecal) phyloseq objects**
...
## **Alpha diversity for Globolus faecal dataset**

*Diversity indices in Globolus faecal dataset*

*Rarefy without replacement and plot richness*
```{r Globolus faecal Rarefy without replacement, cache = T}


# Plot richness for all samples
theme_set(theme_bw())
Glo_faecal_plot_richness <- plot_richness(Glo_faecal, x = 'Sample.name', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
...
Glo_faecal_plot_richness$data$Treatment = factor(Glo_faecal_plot_richness$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_faecal_plot_richness


# Plot richness for all treatments
theme_set(theme_bw())
Glo_faecal_plot_richness1 <- plot_richness(Glo_faecal, x = 'Treatment', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
  theme(axis.text.x = element_text(
    angle = 90,
...
Glo_faecal_plot_richness1$data$Treatment = factor(Glo_faecal_plot_richness1$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_faecal_plot_richness1

ggsave("Glo_faecal_richness_its.png", device = "png", path = "Richness_plot")
...
```{r Globolus faecal measure Richness, Shannon, for the phyloseq object, cache = T}
Glo_faecal_alpha.its <- estimate_richness(Glo_faecal, measures=c("Observed", "Chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "Fisher"))
Glo_faecal_alpha.its


# Add SampleID column to the alpha dataframe
Glo_faecal_alpha.its$Sample <- sample_names(Glo_faecal)

# Merge alpha and Globolus faecal dataset into a single dataframe
Glo_faecal_metad_alpha_its <- merge(psmelt(Glo_faecal), Glo_faecal_alpha.its)

# Write it into a csv file
write.csv(Glo_faecal_metad_alpha_its, file.path("Glo_faecal_alpha_its.csv")) # writes out a .csv file that you can open in excel
```
Let's go ahead and write only metrics with mean and sd in table
```{r Globolus faecal summarize our models, cache = T}
...
# Mean for treatment
Glo_faecal_alphmean.its = aggregate(Glo_faecal_metad_alpha_its[,c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")],list(Glo_faecal_metad_alpha_its$Sample.name, Glo_faecal_metad_alpha_its$Treatment), mean)

# SD for treatment
Glo_faecal_alphsd.its = aggregate(Glo_faecal_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Glo_faecal_metad_alpha_its$Sample.name, Glo_faecal_metad_alpha_its$Treatment), sd)

# SE for treatment
se <- function(x) sd(x) / sqrt(length(x))
Glo_faecal_alphse.its = aggregate(Glo_faecal_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Glo_faecal_metad_alpha_its$Sample.name, Glo_faecal_metad_alpha_its$Treatment), se)

# Rename the columns
colnames(Glo_faecal_alphmean.its) <- c("Sample.name", "Treatment",  "Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")
colnames(Glo_faecal_alphsd.its) <- c("Sample.name", "Treatment", "Observed.sd", "Chao1.sd", "se.chao1.sd", "Shannon.sd", "Simpson.sd", "InvSimpson.sd", "ACE.sd", "se.ACE.sd", "Fisher.sd")
colnames(Glo_faecal_alphse.its) <- c("Sample.name", "Treatment", "Observed.se", "Chao1.se", "se.chao1.se", "Shannon.se", "Simpson.se", "InvSimpson.se", "ACE.se", "se.ACE.se", "Fisher.se")

    
# Merge and write into a csv file
Glo_faecal.mean.sd.se <- Reduce(merge, list(Glo_faecal_alphmean.its, Glo_faecal_alphsd.its, Glo_faecal_alphse.its))
options(digits=2)

write.csv(Glo_faecal.mean.sd.se, file.path("Glo_faecal_alpha_mean_sd_se.csv"))
```
...
data_melted <- reshape2::melt(Glo_faecal.mean.sd.se, id.vars = c("Sample.name", "Treatment")) %>%
...
p_values_list_expanded %>%  kable(digits = c(0, 1, 1, 1, 0, 1)) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F) 
...
p_Glo_faecal <- ggplot(data_melted, aes(x = Treatment, y = value, fill = Treatment)) +
  geom_violin() +
...
ggsave("Glo_faecal_alpha_diversity.svg",width = 20, height = 35, units = "cm")
```

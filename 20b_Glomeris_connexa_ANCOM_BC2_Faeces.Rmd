...
*Group according to sample sources Glomeris faecal pellets*
...
# ALDEX2
```{r ALDEX2, cache = T}
Glo_faecal = subset_taxa(Glo_faecal, Phylum != "unassigned")

significance <- 0.5


# Remove all ASVs that aren't in at least 2 samples or have an abundance of at least 2
Glo_faecal_subset_pairwise_s <- filter_ASVs_by_prevalence(ps_obj = Glo_faecal, 
                                             abund_thresh = 2, 
                                             prev_thresh = 2)
...
for (pair in treatments_to_compare) {
  ...
  subset_data_Glo_faecal <- Glo_faecal_subset_pairwise_s %>%
    subset_samples(Treatment %in% c(treatment1, treatment2))
  
  # Mark rare species -------------------------------------------------------
  Glo_faecal_subset_glom <- tax_glom(subset_data_Glo_faecal,
                               "Phylum",
                               NArm = TRUE)
  Glo_faecal_subset_glom_rel <-
    transform_sample_counts(Glo_faecal_subset_glom, function(x)
      x / sum(x))
  Glo_faecal_subset_glom_rel_DF <- psmelt(Glo_faecal_subset_glom_rel)
  Glo_faecal_subset_glom_rel_DF$Phylum %<>% as.character()

  # Group dataframe by Phylum, calculate median rel. abundance
  Glo_faecal_subset_glom_rel_DF %>%
    group_by(Phylum) %>%
  ...
  Glo_faecal_subset_glom_rel_DF[Glo_faecal_subset_glom_rel_DF$Phylum %in% Rare_phyla,]$Phylum <-
    'Rare'
...
Glo_faecal_subset_glom_rel_DF %>%
  ...
  ALDEx2plot_pairwise_Glo_faecal <- calc_ALDEx2(
    physeq_obj = subset_data_Glo_faecal,
    vars2test = "Treatment",
  ...
  result_name_Glo_faecal <- paste(treatment1, "_vs_", treatment2)
  results_list_Glo_faecal[[result_name_Glo_faecal]] <- ALDEx2plot_pairwise
}


saveRDS(results_list_Glo_faecal, "RDS/Glo_faecal_ALDEx_results_list.Rds")

results_list_Glo_faecal <- readRDS("RDS/Glo_faecal_ALDEx_results_list.Rds")
...
ALDEX_summary_Glo_faecal <- purrr::map_dfr(results_list_Glo_faecal, ~.x[, common_columns], .id = "Comparison")
#ALDEX_summary <- ALDEX_summary %>% filter(Phylum == "NA")

# # Save
write_csv(ALDEX_summary_Glo_faecal, "ANCOM_BC_Table/Glo_faecal_ALDEX_summary.csv")


ALDEX_summary_Glo_faecal %>%
  filter(Significance == "Pass") %>%
  dplyr::select(Comparison, OTU, baseMean, effect, Phylum, Class, Order, Family, Genus) %>%
  arrange(desc(abs(effect))) ->
  ALDEx2plot_pairwise_results_Glo_faecal


# # Save
write_csv(ALDEx2plot_pairwise_results_Glo_faecal, "ANCOM_BC_Table/Glo_faecal_ALDEX_summary_Significant.csv")
...
ALDEX_summary_Glo_faecal$Comparison <- factor(ALDEX_summary_Glo_faecal$Comparison, levels = custom_order)


p1_Glo_hindgut <- plot_ALDEx_tax(ALDEX_summary_Glo_faecal,
                 OTU_labels = FALSE,
                 sig_level = significance) +
...
ggsave("ANCOM_BC_Fig/ALDEX_Glo_faecal.svg", width = 35, height = 25, units = "cm")
```



##**Faeces: Analysis of Compositions of Microbiomes with Bias Correction (ANCOM-BC)**
```{r Globolus Hindgut ANCOMBC, cache = T}
# For faeces
set.seed(123)

# remove all ASVs that aren't in at least 2 samples or have at least abundance of 2
Glo_faecal_sub <- filter_ASVs_by_prevalence(ps_obj = Glo_faecal, 
                                             abund_thresh = 2, 
                                             prev_thresh = 2)

sample_data(Glo_faecal_sub)$Treatment <- as.factor(sample_data(Glo_faecal_sub)$Treatment)

sample_data(Glo_faecal_sub)$Treatment <- factor(sample_data(Glo_faecal_sub)$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))


# Coerce a phyloseq object to a TreeSummarizedExperiment
tse_Glo_faecal_sub <- makeTreeSummarizedExperimentFromPhyloseq(Glo_faecal_sub)

# Access the metadata slot of the TreeSummarizedExperiment
tse_metadata <- colData(tse_Glo_faecal_sub)
...
colData(tse_Glo_faecal_sub) <- tse_metadata
...
tse_Glo_faecal_sub <- phyloseq_to_tse_safe(Glo_faecal_sub,
                                            test_condition = "Treatment",
                                            ref_level = "Control")


# Run ANCOM-BC at the genus level and only including the prevalent genera
Glo_faecal_sub_ancombc2 <- ancombc2(data = tse_Glo_faecal_sub,
               tax_level = "Genus",
                assay_name = "counts",
...
saveRDS(Glo_faecal_sub_ancombc2, "RDS/Glo_faecal_sub_ancombc2.Rds")

Glo_faecal_sub_ancombc2 <- readRDS("RDS/Glo_faecal_sub_ancombc2.Rds")




res_prim = Glo_faecal_sub_ancombc2$res

# # Save
 write_csv(res_prim, "ANCOM_BC_Table/Glo_faecal_sub_ancombc2.csv")


########################################################## ANCOM-BC2 global test ################################################################

res_global = Glo_faecal_sub_ancombc2$res_global

write_csv(res_global, "ANCOM_BC_Table/Glo_faecal_sub_ancombc2_global.csv")
...
ggsave("ANCOM_BC_Fig/Glo_faecal_sub_ANCOM-BC2.svg",width = 23, height = 30, units = "cm")
...
res_pair = Glo_faecal_sub_ancombc2$res_pair
write_csv(res_pair, "ANCOM_BC_Table/Glo_faecal_sub_ancombc2_pairwise.csv")
...
ggsave("ANCOM_BC_Fig/Glo_faecal_sub_ANCOM-BC2_multiple_pairwise.svg",width = 59.65, height = 50, units = "cm")
```

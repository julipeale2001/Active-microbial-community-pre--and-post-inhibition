...
## **Read the data and create both Globolus (hindgut and faecal) phyloseq objects**
...
# ALDEX2
```{r ALDEX2, cache = T}
Glo_hindgut = subset_taxa(Glo_hindgut, Phylum != "unassigned")

significance <- 0.5


# Remove all ASVs that aren't in at least 2 samples or have an abundance of at least 2
Glo_hindgut_subset_pairwise_s <- filter_ASVs_by_prevalence(ps_obj = Glo_hindgut, 
                                             abund_thresh = 2, 
                                             prev_thresh = 2)
...
treatments_to_compare <- list(
  ...
  c("Antibiotics (5X)", "Control"),
  c("Antibiotics (2X)", "Control"),
  c("Antibiotics (5X)", "Sterile feed"),
  c("Antibiotics (2X)", "Sterile feed"),
  c("Antibiotics (2X)", "Antibiotics (5X)")
)

# Create an empty list to store the results
results_list_Glo_hindgut <- list()
...
for (pair in treatments_to_compare) {
  ...
  subset_data_Glo_hindgut <- Glo_hindgut_subset_pairwise_s %>%
    subset_samples(Treatment %in% c(treatment1, treatment2))
  
  # Mark rare species -------------------------------------------------------
  Glo_hindgut_subset_glom <- tax_glom(subset_data_Glo_hindgut,
                               "Phylum",
                               NArm = TRUE)
  Glo_hindgut_subset_glom_rel <-
    transform_sample_counts(Glo_hindgut_subset_glom, function(x)
      x / sum(x))
  Glo_hindgut_subset_glom_rel_DF <- psmelt(Glo_hindgut_subset_glom_rel)
  Glo_hindgut_subset_glom_rel_DF$Phylum %<>% as.character()

  # Group dataframe by Phylum, calculate median rel. abundance
  Glo_hindgut_subset_glom_rel_DF %>%
    group_by(Phylum) %>%
  ...
  Glo_hindgut_subset_glom_rel_DF[Glo_hindgut_subset_glom_rel_DF$Phylum %in% Rare_phyla,]$Phylum <-
    'Rare'
...
# Run calc_ALDEx2 for the pair
  ALDEx2plot_pairwise_Glo_hindgut <- calc_ALDEx2(
    physeq_obj = subset_data_Glo_hindgut,
    vars2test = "Treatment",
...
# Store the result in the list with the treatment names
  result_name_Glo_hindgut <- paste(treatment1, "_vs_", treatment2)
  results_list_Glo_hindgut[[result_name_Glo_hindgut]] <- ALDEx2plot_pairwise_Glo_hindgut
}


saveRDS(results_list_Glo_hindgut, "RDS/Glo_hindgut_ALDEx_results_list.Rds")

results_list_Glo_hindgut <- readRDS("RDS/Glo_hindgut_ALDEx_results_list.Rds")
...
ALDEX_summary_Glo_hindgut <- purrr::map_dfr(results_list_Glo_hindgut, ~.x[, common_columns], .id = "Comparison")
#ALDEX_summary <- ALDEX_summary %>% filter(Phylum == "NA")

# # Save
write_csv(ALDEX_summary_Glo_hindgut, "ANCOM_BC_Table/Glo_hindgut_ALDEX_summary.csv")


ALDEX_summary_Glo_hindgut %>%
  filter(Significance == "Pass") %>%
  dplyr::select(Comparison, OTU, baseMean, effect, Phylum, Class, Order, Family, Genus) %>%
  arrange(desc(abs(effect))) ->
  ALDEx2plot_pairwise_results_Glo_hindgut


# # Save
write_csv(ALDEx2plot_pairwise_results_Glo_hindgut, "ANCOM_BC_Table/Glo_hindgut_ALDEX_summary_Significant.csv")
...
custom_order <- c(
  ...
  "Antibiotics (5X) _vs_ Control",
  "Antibiotics (2X) _vs_ Control",
  "Antibiotics (5X) _vs_ Sterile feed",
  "Antibiotics (2X) _vs_ Sterile feed",
  "Antibiotics (2X) _vs_ Antibiotics (5X)"
)

# Convert "Comparison" to a factor with the custom order
ALDEX_summary_Glo_hindgut$Comparison <- factor(ALDEX_summary_Glo_hindgut$Comparison, levels = custom_order)


p1_Glo_hindgut <- plot_ALDEx_tax(ALDEX_summary_Glo_hindgut,
                 OTU_labels = FALSE,
                 sig_level = significance) +
 
  facet_wrap(~ Comparison, scales = "free_y")  # Facet by "Comparison"

print(p1_Glo_hindgut)

ggsave("ANCOM_BC_Fig/ALDEX_Glo_hindgut.svg", width = 35, height = 25, units = "cm")
```
...
tse_Glo_hindgut_sub <- phyloseq_to_tse_safe(Glo_hindgut_sub,
...
df_fig_global = df_Treatment %>%
    ...
    dplyr::mutate(`lfc_Sterile feed` = `lfc_TreatmentSterile feed`,
                  ...
                  `lfc_Antibiotics (2X)` = `lfc_TreatmentAntibiotics (2X)`, color = ifelse(passed_ss == 1, "aquamarine3", "black")) %>%
    dplyr::transmute(taxon, 
                     `Sterile feed vs. Control` = round(`lfc_Sterile feed`, 2), 
                     `Antibiotics (5X) vs. Control` = round(`lfc_Antibiotics (5X)`, 2),
                     `Antibiotics (2X) vs. Control` = round(`lfc_Antibiotics (2X)`, 2), color = color) %>%
...
res_pair = Glo_hindgut_sub_ancombc2$res_pair

write_csv(res_pair, "ANCOM_BC_Table/Glo_hindgut_sub_ancombc2_pairwise.csv")

df_fig_pair1 = res_pair %>%
    dplyr::filter(`diff_TreatmentSterile feed` == 1 |
                      `diff_TreatmentAntibiotics (5X)` == 1 | `diff_TreatmentAntibiotics (2X)` == 1 |
                      `diff_TreatmentAntibiotics (5X)_TreatmentSterile feed` == 1 | `diff_TreatmentAntibiotics (2X)_TreatmentSterile feed` == 1
                  | `diff_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)` == 1) %>%
    dplyr::mutate(lfc1 = ifelse(`diff_TreatmentSterile feed` == 1, 
                                round(`lfc_TreatmentSterile feed`, 2), 0),
                  lfc2 = ifelse(`diff_TreatmentAntibiotics (5X)` == 1, 
                                round(`lfc_TreatmentAntibiotics (5X)`, 2), 0),
                  lfc3 = ifelse(`diff_TreatmentAntibiotics (2X)` == 1, 
                                round(`lfc_TreatmentAntibiotics (2X)`, 2), 0),
                  lfc4 = ifelse(`diff_TreatmentAntibiotics (5X)_TreatmentSterile feed` == 1, 
                                round(`lfc_TreatmentAntibiotics (5X)_TreatmentSterile feed`, 2), 0),
                  lfc5 = ifelse(`diff_TreatmentAntibiotics (2X)_TreatmentSterile feed` == 1, 
                                round(`lfc_TreatmentAntibiotics (2X)_TreatmentSterile feed`, 2), 0),
                  lfc6 = ifelse(`diff_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)` == 1, 
                                round(`lfc_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)`, 2), 0)) %>%
...
df_fig_pair2 = res_pair %>%
    dplyr::filter(`diff_TreatmentSterile feed` == 1 |
                      `diff_TreatmentAntibiotics (5X)` == 1 | `diff_TreatmentAntibiotics (2X)` == 1 |
                      `diff_TreatmentAntibiotics (5X)_TreatmentSterile feed` == 1 | `diff_TreatmentAntibiotics (2X)_TreatmentSterile feed` == 1 | `diff_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)` == 1) %>%
    dplyr::mutate(lfc1 = ifelse(`passed_ss_TreatmentSterile feed` == 1 & `diff_TreatmentSterile feed` == 1, 
                                "aquamarine3", "black"),
                  lfc2 = ifelse(`passed_ss_TreatmentAntibiotics (5X)` == 1 & `diff_TreatmentAntibiotics (5X)` == 1, 
                                "aquamarine3", "black"),
                  lfc3 = ifelse(`passed_ss_TreatmentAntibiotics (2X)` == 1 & `diff_TreatmentAntibiotics (2X)` == 1, 
                                "aquamarine3", "black"),
                  lfc4 = ifelse(`passed_ss_TreatmentAntibiotics (5X)_TreatmentSterile feed` == 1 & `diff_TreatmentAntibiotics (5X)_TreatmentSterile feed` == 1, 
                                "aquamarine3", "black"),
                  lfc5 = ifelse(`passed_ss_TreatmentAntibiotics (2X)_TreatmentSterile feed` == 1 & `diff_TreatmentAntibiotics (2X)_TreatmentSterile feed` == 1, 
                                "aquamarine3", "black"),
                  lfc6 = ifelse(`passed_ss_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)` == 1 & `diff_TreatmentAntibiotics (2X)_TreatmentAntibiotics (5X)` == 1, 
                                "aquamarine3", "black")) %>%
    tidyr::pivot_longer(cols = lfc1:lfc6, 
...
df_fig_pair$group = dplyr::recode(df_fig_pair$group, 
                          ...
                          `lfc2` = "Antibiotics (5X) - Control",
                          `lfc3` = "Antibiotics (2X) - Control",
                          `lfc4` = "Antibiotics (5X) - Sterile feed",
                          `lfc5` = "Antibiotics (2X) - Sterile feed",
                          `lfc6` = "Antibiotics (2X) - Antibiotics (5X)")
df_fig_pair$group = factor(df_fig_pair$group, 
                          levels = c("Sterile feed - Control",
                                     "Antibiotics (5X) - Control",
                                     "Antibiotics (2X) - Control",
                                     "Antibiotics (5X) - Sterile feed",
                                     "Antibiotics (2X) - Sterile feed",
                                     "Antibiotics (2X) - Antibiotics (5X)"))
...
fig_pair = df_fig_pair %>%
    ...
    scale_fill_gradientn(low = "blue", high = "red", mid = "white", 
...
ggsave("ANCOM_BC_Fig/Glo_hindgut_sub_ANCOM-BC2_multiple_pairwise.svg",width = 56.6, height = 22.4, units = "cm")
```

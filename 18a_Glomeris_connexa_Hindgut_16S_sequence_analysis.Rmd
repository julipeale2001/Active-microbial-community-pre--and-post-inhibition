---
title: "Amplicon Sequence Analyses for Glomeris Hindgut Antibiotic Experiment (using Silva tax)"
subtitle: "Glomeris Hindgut Antibiotic Experiment" 
author: "Julius Nweze"
date: "`r Sys.Date()`"
output:
...
### **Glomeris hingdut analysis**


# *Glomeris hindgut sample dataset summary*

*As the first analysis, we will look at the distribution of read counts from Glomeris hingdut samples*
```{r Glomeris hindgut sample summary, cache = T}
# Make a data frame with a column for the read counts of each Glomeris hingdut sample
sample_sum_df <- data.frame(sum = sample_sums(Glo_hindgut))

# Histogram of Glomeris hingdut sample read counts
ggplot(sample_sum_df, aes(x = sum)) + 
  geom_histogram(color = "black", fill = "indianred", binwidth = 2500) +
  ggtitle("Distribution of Glomeris hindgut sample sequencing depth") + 
  xlab("Read counts") +
  theme(axis.title.y = element_blank())
# mean, max and min of Glomeris hingdut sample read counts
smin <- min(sample_sums(Glo_hindgut))
print(smin)
smean <- mean(sample_sums(Glo_hindgut))
print(smean)
smax <- max(sample_sums(Glo_hindgut))
print(smax)
```
The distribution of read counts from Glomeris faecal samples showed that the minimum, mean and maximum to be 22082, 41225.17 and  53139 respectively.



*Show available ranks in the Glomeris hindgut dataset*
```{r Glomeris hindgut available ranks in the dataset, cache = T}
# Show available ranks in the dataset
rank_names(Glo_hindgut)

# Create table, number of features for each phyla
table(tax_table(Glo_hindgut)[, "Phylum"], exclude = NULL)
```
This shows four phyla for which only 1 feature was observed. Those may be worth filtering, and we’ll check that next. First, notice that in this case, 146 features were annotated with a Phylum of NA. These features are probably artifacts in a dataset like this, and should be removed.



*The following ensures that features with ambiguous phylum annotation are also removed from the Glomeris hindgut dataset.*
```{r Glomeris hindgut removal of ambiguous phylum annotation, cache = T}
Glo_hindgut0 <- subset_taxa(Glo_hindgut, !is.na(Phylum) & !Phylum %in% c("", "uncharacterized"))
```



*Exploring the feature prevalence in the Glomeris hindgut dataset, which we will define here as the number of samples in which a taxa appears at least once.*
```{r Glomeris hindgut exploration of the feature prevalence in the dataset, cache = T}
# Compute prevalence of each feature, store as data.frame
prevdf = apply(X = otu_table(Glo_hindgut0),
                 MARGIN = ifelse(taxa_are_rows(Glo_hindgut0), yes = 1, no = 2),
                 FUN = function(x){sum(x > 0)})

# Add taxonomy and total read counts to this data.frame
prevdf = data.frame(Prevalence = prevdf,
                      TotalAbundance = taxa_sums(Glo_hindgut0),
                      tax_table(Glo_hindgut0))
```



*Because of the phyla that are comprised of mostly low-prevalence features, let us compute the total and average prevalences of the features in each Glomeris hindgut dataset phylum.*
```{r Glomeris hindgut the total and average prevalences of the features in each phylum, cache = T}
plyr::ddply(prevdf, "Phylum", function(df1){cbind(mean(df1$Prevalence),sum(df1$Prevalence))})
```
Abditibacteriota, Armatimonadota, Campylobacterota, Entotheonellaeota, FCPU426, Hydrogenedentota, Latescibacterota, Nitrospirota, Patescibacteria, RCP2-54, and Sumerlaeota should be filtered from the dataset.



# *Filter low-prevalent phyla in the Glomeris hindgut dataset.*
```{r Glomeris hindgut phyla to filter, cache = T}
# Define phyla to filter
filterPhyla = c("Abditibacteriota", "Armatimonadota", "Campylobacterota", "Entotheonellaeota", "FCPU426", "Fibrobacterota", "Hydrogenedentota", "Latescibacterota", "Nitrospirota", "Patescibacteria", "RCP2-54", "Sumerlaeota")
# Filter entries with unidentified Phylum.
Glo_hindgut1 = subset_taxa(Glo_hindgut0, !Phylum %in% filterPhyla)
Glo_hindgut1
```



*Prevalence Filtering in the Glomeris hindgut dataset*


*First, explore the relationship of prevalence and total read count for each Glomeris faecal dataset feature.*
```{r Glomeris hindgut prevalence filtering, cache = T}
# Subset to the remaining phyla
prevdf1 = subset(prevdf, Phylum %in% get_taxa_unique(Glo_hindgut1, "Phylum"))
ggplot(prevdf1, aes(TotalAbundance, Prevalence / nsamples(Glo_hindgut0),color=Phylum)) +
...
*The following uses five percent (5%) of all samples as the prevalence threshold in the Glomeris hindgut dataset.*
```{r Glomeris hindgut 5% prevalence filtering, cache = T}
#  Define prevalence threshold as 5% of total samples
prevalenceThreshold = 0.05 * nsamples(Glo_hindgut0)
prevalenceThreshold

# Execute prevalence filter, using `prune_taxa()` function
keepTaxa = rownames(prevdf1)[(prevdf1$Prevalence >= prevalenceThreshold)]
Glo_hindgut2 = prune_taxa(keepTaxa, Glo_hindgut0)
...
## **Exploring biodiversity: visualization in the Glomeris hindgut dataset**

*Normalize number of reads in each Glomeris hindgut dataset sample using median sequencing depth.*
```{r Glomeris hindgut normalization of reads, cache = T}
total = median(sample_sums(Glo_hindgut2))
standf = function(x, t=total) round(t * (x / sum(x)))
Glo_hindgut2 = transform_sample_counts(Glo_hindgut2, standf)

#Changing order of levels
sample_data(Glo_hindgut2)$Treatment<-factor(sample_data(Glo_hindgut2)$Treatment, levels=c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
levels(sample_data(Glo_hindgut2)$Treatment)
```



# *Glomeris hindgut dataset bar graphs*
*Basic Glomeris hindgut  dataset bar graph based on Phylum*
```{r Glomeris hindgut Basic bar graph based on Phylum, cache = T}

plot_bar(Glo_hindgut2, fill = "Phylum") + facet_wrap(~ Treatment, scales =
"free_x", nrow = 1) + theme(legend.position="bottom")
```



# *Make the Glomeris hindgut dataset bar graph nicer by removing OTUs boundaries. This is done by adding ggplot2 modifier.*
```{r adding ggplot2 modifier Glomeris hindgut, cache = T}
 plot_bar(Glo_hindgut2, fill = "Phylum") + 
...
*Regroup together according to Glomeris hindgut's treatments*
```{r regroup together according to samples types Glomeris hindgut, cache = T}
# Merge by treatment
Glo_hindgut_fraction <- merge_samples(Glo_hindgut2, "Treatment")
# Repair the merged values associated with each surface after merge.
sample_data(Glo_hindgut_fraction)$Treatment <- levels(sample_data(Glo_hindgut_fraction)$Treatment)

# Flip

sample_data(Glo_hindgut_fraction)$Treatment <- sample_names(Glo_hindgut_fraction)
ps2.Glo_hindgut_fraction <- transform_sample_counts(Glo_hindgut_fraction,  
                                    function(x) x/sum(x) * 100)
p <- plot_bar(ps2.Glo_hindgut_fraction, fill = "Phylum") + 
...
# **Heatmaps for Glomeris hindgut dataset**
...
```{r filtering taxa in Glomeris hindgut, cache = T}
# Relative abundance
Glo_hindgut_abund <- filter_taxa(Glo_hindgut2, function(x) sum(x > total*0.05) > 0, TRUE)
Glo_hindgut_abund
```

*New heatmap for Glomeris hindgut dataset*
```{r Glomeris tsv table and heatmap, cache = T}
plot_heatmap(Glo_hindgut_abund, method = "PCoA", distance = "bray")
```



*Using different distances and different multivaraite methods (Jaccard distance and PCoA and label ASVs with Class, order by Class) in the Glomeris hingdut dataset. We can also change the Palette (the default palette is a bit ugly…).*
```{r Glomeris hindgut heatmap with different distances and different multivaraite methods, cache = T}
plot_heatmap(Glo_hindgut_abund, method = "PCoA", distance = "jaccard",
...
**Phylum to Genus Distributions in the Glomeris hindgut dataset using the most abundant ASVs** 

*Phylum distribution in Glomeris hindgut dataset using top phyla*
```{r most abundant ASVs for phylum distribution Glomeris hindgut, cache = T}
# Let us use 0.5%
Glo_hindgut_abundant <- filter_taxa(Glo_hindgut2, function(x) sum(x > total*0.005) > 0, TRUE)
Glo_hindgut_abundant

# Creating 100% abundance stacked bar plots with all the phylum

Glo_hindgut_a <- ggplot(data = psmelt(Glo_hindgut_abundant), mapping = aes_string(x = "Treatment",y = "Abundance")) + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="fill")
Glo_hindgut_a$data$Treatment <- factor(Glo_hindgut_a$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_hindgut_a
```
...
*Agglomerate taxa*
```{r Agglomerate taxa most abundant ASVs for phylum distribution Glomeris hindgut, cache = T}

Glo_hindgut_GMPR_rel <- transform_sample_counts(Glo_hindgut1, function(x) x / sum(x) )

library(plyr)
Glo_hindgut_GMPR_rel %>% 
  sample_data() %>% 
  arrange(Treatment) %>% 
  .$Sample.name ->
  Sample_order
...
Glo_hindgut_100 <-
  prune_taxa(names(sort(taxa_sums(Glo_hindgut_GMPR_rel), TRUE)[1:100]), Glo_hindgut_GMPR_rel)
plot_heatmap(
  Glo_hindgut_100,
  method = NULL,
  distance = NULL,
...
```{r Checking agglo taxa most abundant ASVs for phylum distribution Glomeris hindgut, cache = T}

Glo_hindgut_glom <- tax_glom(Glo_hindgut1, 
                             "Phylum", 
                             NArm = TRUE)
Glo_hindgut_glom_rel <- transform_sample_counts(Glo_hindgut_glom, function(x) x / sum(x)) 
Glo_hindgut_glom_rel_DF <- psmelt(Glo_hindgut_glom_rel)
Glo_hindgut_glom_rel_DF$Phylum %<>% as.character()

# group dataframe by Phylum, calculate median rel. abundance
detach(package:plyr)    
library(dplyr) # OR df1 <- ddply(Glo_hindgut_glom_rel_DF, .(Phylum), summarise, median = median(Abundance))
Glo_hindgut_glom_rel_DF %>%
  group_by(Phylum) %>%
  summarise(median = median(Abundance)) ->
  medians
...
Glo_hindgut_glom_rel_DF[Glo_hindgut_glom_rel_DF$Phylum %in% Rare_phyla, ]$Phylum <- 'Rares'
# re-group
Glo_hindgut_glom_rel_DF %>%
  group_by(Sample, Sample.name, Treatment, Phylum, Sample.type) %>%
  summarise(Abundance = sum(Abundance)) ->
  Glo_hindgut_glom_rel_DF_2plot

# ab.taxonomy$Freq <- sqrt(ab.taxonomy$Freq)
Glo_hindgut_glom_rel_DF_2plot$Phylum %<>% sub("unclassified", "Unclassified", .)
Glo_hindgut_glom_rel_DF_2plot$Phylum %<>% sub("uncultured", "Unclassified", .)
...
Glo_hindgut_glom_rel_DF_2plot$Phylum %<>% 
  factor(., levels = Taxa_rank$Phylum) %>% 
  fct_relevel(., "Rares", after = Inf)
  
p_taxa_box <-
  ggplot(Glo_hindgut_glom_rel_DF_2plot, aes(x = Phylum, y = (Abundance*100), fill = Treatment)) + scale_y_continuous(limits=c(0,100)) +
...
Glo_hindgut_d <- ggplot(Glo_hindgut_glom_rel_DF_2plot, mapping = aes(x=Phylum, y=Abundance, fill = Treatment)) + geom_boxplot() +  scale_color_hue(l=40, c=35) + geom_point(aes(colour = Treatment), position = position_jitterdodge(dodge.width = 1), alpha = 1 / 2, stroke = 0, size = 2) + scale_y_continuous(limits=c(0,1)) +
  theme(axis.text.x = element_text(
    angle = 45,
    vjust = 0.9,
    hjust = 0.9
  )) 
Glo_hindgut_d$data$Treatment <- factor(Glo_hindgut_d$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
print(Glo_hindgut_d)
```
...
```{r Creating 100% abundance stacked bar plots for most abundant ASVs for phylum distribution Glomeris hindgut, cache = T}

Glo_hindgut_b <- ggplot(data = Glo_hindgut_glom_rel_DF_2plot, mapping = aes_string(x = "Treatment", y = "Abundance")) + geom_bar(aes(color = Phylum, fill  = Phylum), stat = "identity", position= "fill") + ylab("Relative Abundance") +
  theme( text = element_text(size = 18),  
axis.text = element_text(size = 18)) + coord_flip()
Glo_hindgut_b$data$Treatment <- factor(Glo_hindgut_b$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_hindgut_b
```
Proteobacteria are the most abundant Phylum in all the treatment groups, followed by Bacteriodota. 




*Genus distribution in Glomeris hindgut dataset using ASVs that represent at least 10% of reads in at least one sample*
```{r most abundant ASVs for genus distribution Glomeris hindgut, cache = T}
# Let us use 10%
Glo_hindgut_abundant <- filter_taxa(Glo_hindgut2, function(x) sum(x > total*0.1) > 0, TRUE)
Glo_hindgut_abundant
```

*Summarizing relative family abundance in Glomeris dataset*

Let’s look at the agglomerated Genus

```{r Checking agglo taxa most abundant ASVs for Genus distribution Glomeris hindgut, cache = T}
...
Glo_hindgut_glom_rel <- transform_sample_counts(Glo_hindgut_glom, function(x) x / sum(x)) 
Glo_hindgut_glom_rel_DF <- psmelt(Glo_hindgut_glom_rel)
Glo_hindgut_glom_rel_DF$Genus %<>% as.character()

# group dataframe by Genus, calculate median rel. abundance
#detach(package:plyr)    
library(dplyr) # OR df1 <- ddply(Glo_hindgut_glom_rel_DF, .(Genus), summarise, median = median(Abundance))
Glo_hindgut_glom_rel_DF %>%
  group_by(Genus) %>%
  summarise(median = median(Abundance)) ->
  medians
...
# find Phyla whose rel. abund. is less than 0.5%
Other_phyla <- medians[medians$median <= 0.03, ]$Genus

# change their name to "Rares"
Glo_hindgut_glom_rel_DF[Glo_hindgut_glom_rel_DF$Genus %in% Other_phyla, ]$Genus <- 'Rares'
# re-group
Glo_hindgut_glom_rel_DF %>%
...
# ab.taxonomy$Freq <- sqrt(ab.taxonomy$Freq)
Glo_hindgut_glom_rel_DF_2plot$Genus %<>% sub("unclassified", "Unclassified", .)
Glo_hindgut_glom_rel_DF_2plot$Genus %<>% sub("uncultured", "Unclassified", .)
set.seed(1)
Glo_hindgut_glom_rel_DF_2plot %>% 
  group_by(Sample) %>% 
  filter(Genus == "Rares") %>% 
...
Glo_hindgut_glom_rel_DF_2plot %>% 
  ...
  summarise(sum.Taxa = sum(Abundance)) %>% 
  arrange(desc(sum.Taxa)) -> Taxa_rank
Glo_hindgut_glom_rel_DF_2plot$Genus %<>% 
  factor(., levels = Taxa_rank$Genus) %>% 
  fct_relevel(., "Rares", after = Inf)
  
p_taxa_box <-
  ggplot(Glo_hindgut_glom_rel_DF_2plot, aes(x = Genus, y = (Abundance*100), fill = Treatment)) + scale_y_continuous(limits=c(0,100)) +
...
## Using it to make joined Boxplot
Glo_hindgut_d <- ggplot(Glo_hindgut_glom_rel_DF_2plot, mapping = aes(x=Genus, y=Abundance, fill = Treatment)) + geom_boxplot() +  scale_color_hue(l=40, c=35) + geom_point(aes(colour = Treatment), position = position_jitterdodge(dodge.width = 1), alpha = 1 / 2, stroke = 0, size = 2) + scale_y_continuous(limits=c(0,1)) +
  theme(axis.text.x = element_text(
    angle = 45,
    vjust = 0.9,
    hjust = 0.9
...
*Creating 100% abundance stacked bar plots with all the families in Glomeris dataset*

```{r Creating 100% abundance stacked bar plots for most abundant ASVs for Genus distribution Glomeris hindgut, cache = T}
grid.colour = c(Citrobacter  = "#006400", Desulfovibrio  = "#A52A2A", Bacteroides  = "#008080", Alistipes  = "#8A2BE2", Candidatus_Symbiothrix  = "#808000", Candidatus_Methanoplasma = "#00FF00", Pseudomonas = "#00FFFF", Dysgonomonas  = "#FF00FF", Achromobacter  = "#800000", Thauera = "#FF6347", Klebsiella = "#FFFF00", Rares = "#C0C0C0")

Glo_hindgut_glom_rel_DF_2plot %>%
group_by(Treatment, Genus) %>%
summarize(Abundance = sum(Abundance)) %>%
ggplot(mapping = aes_string(x = "Treatment", y = "Abundance")) + geom_bar(aes(fill  = Genus), stat = "identity", position= "fill") + ylab("Relative Abundance") + scale_fill_manual(values = grid.colour) ->
Glo_hindgut_b
Glo_hindgut_b$data$Treatment <- factor(Glo_hindgut_b$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_hindgut_b
```
Here, the predominant Genera:
Control - Dysgonomonas followed by Aeromonas
Sterile-fed - Candidatus_Symbiothrix followed by Microvirga
Antibiotics (5X) - Pseudomonas followed by Achromobacter
Antibiotics (2X) - Pseudomonas followed by Microvirga

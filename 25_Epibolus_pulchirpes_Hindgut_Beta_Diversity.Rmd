...
# pairwise_adonis
pairwise_adonis <- function(x, factors, sim.function = "vegdist", sim.method = "horn", 
                           p.adjust.m = "BH", reduce = NULL) 
{
  require(vegan)
  # Taken from: https://github.com/pmartinezarbizu/pairwiseAdonis
...
## **Epibolus hindgut β-diversity indices**

*Distance based matrices such as Bray-Curtis, Unifrac are commonly used and visualized using ordination methods.* 
*Note: The Multidimensionnal Scaling (MDS) or Principal Coordinates Analysis (PCoA) *
```{r Epibolus hindgut β-diversity indices, cache = T}

# Since, there is no phylogenetic tree for this data set, Bray-Curtis distance will be calculated.
Epi_hindgut.rel <- microbiome::transform(Epi_hindgut2, "compositional")

Epi_hindgut.ord_pcoa_bray <- ordinate(Epi_hindgut.rel, "CAP", "bray", formula =  ~ Treatment) # Produce uncorrelated data

# we test the impact of the treatment 
set.seed(10000)
anova(Epi_hindgut.ord_pcoa_bray, by="terms" , permu=999)

# testing various factors (increase the permutation for real data)
anova(Epi_hindgut.ord_pcoa_bray)

# calculate R squared values

RsquareAdj (Epi_hindgut.ord_pcoa_bray)


# # chech which axis are explaining how mauch variation
plot_scree(Epi_hindgut.ord_pcoa_bray) + theme_bw() # Axis 1 and 2 are of interest.
# A scree plot is an important tool for any ordination method, as the relative importance of axes can vary widely from one dataset to another.


Epi_hindgut_beta.ps1 <- phyloseq::plot_ordination(Epi_hindgut.rel,
                            Epi_hindgut.ord_pcoa_bray, 
                            color="Treatment", 
                            label = "Sample.name") + 
  geom_point(size= 8) + 
  theme(plot.title = element_text(hjust = 0, size = 38))+
  scale_colour_manual(values = pom4, name = "") +
  scale_fill_manual(values = pom4, name = "") 

Epi_hindgut_beta.ps1 <- Epi_hindgut_beta.ps1 + theme_bw(base_size = 30) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  geom_line() 
Epi_hindgut_beta.ps1
ggsave("Epi_hindgut_CAP.svg",width = 30, height = 20, units = "cm")
# Beta diversity is the difference in microbial composition between samples. It compares samples and asks what are the differences in microbial composition between the samples. Methods:
# 1. Principal Coordinate Analysis Plot (PCoA) is used for visualization of the data in the distance matrix in a 3D plot. The distance matrix is transformed into a new set of orthogonal axes where the first axis (PC1) can be used to explain the maximum amount of variation present in the dataset, followed by the second axis (PC2), third (PC3), etc.
# 2. The Jaccard distance is based on the presence or absence of species (does not include abundance information). Values are from 0 to 1. A value of 0 means both samples share the exact same species and a value of 1 means both samples have no species in common.
# 3. Bray-Curtis - takes abundance into account; non-phylogeny based. Values are from 0 to 1. A value of 0 means that two samples share the same species at the exact same abundances. A value of 1 means two samples have completely different species abundances.
```
This ordination plot shows that axis 1 explains 24.7 % of the variation among the treatments. The Control and Sterile-fed tend to be ordinated closer together, showing smaller dissimilarity. The two antibiotic treatments are ordinated together and a bit far from the other treatment groups. The sterile group is closer to the Antibiotic-treated groups than the Control.
...
```{r Epibolus hindgut Permanova for beta diversity, cache = T}
set.seed(10000)
# Calculate bray curtis distance matrix
Epi_hindgut_erie_bray <- phyloseq::distance(Epi_hindgut2, method = "bray", formula =  ~ Treatment)

# make a data frame from the sample_data
sampledf <- data.frame(sample_data(Epi_hindgut2))

# Adonis test
adonis2(Epi_hindgut_erie_bray ~ Treatment, data = sampledf)
    
# Homogeneity of dispersion test
beta <- betadisper(Epi_hindgut_erie_bray, sampledf$Treatment)
permutest(beta)
par(mfrow=c(1,1))
plot(beta,
...
Epi_hindgut.ord_pcoa_bray <- ordinate(Epi_hindgut2, "CAP", "bray", formula =  ~ Treatment) # Produce uncorrelated data

# we test the impact of the treatment 

(mod1 <- adonis2(phyloseq::distance(Epi_hindgut2, method = "bray") ~ Treatment,
  data = get_variable(Epi_hindgut2),
  permutations = 9999
))

mod_final_pairwise_all <- pairwise_adonis(
  t(otu_table(Epi_hindgut2)),
  sample_data(Epi_hindgut2)$Treatment,
  # pull(Ps_obj_median, Var3.Var4),
  sim.function = "vegdist",
  sim.method = "bray",
  p.adjust.m = "BH"
)
print(mod_final_pairwise_all)
...
# testing various treatnets (increase the permutation for real data)
anova(Epi_hindgut.ord_pcoa_bray)




# First: Define pairwise comparisons (using preformed script)
pairwise_adonis(x = t(otu_table(Epi_hindgut2)), factor = get_variable(Epi_hindgut2, "Treatment"), sim.function = "vegdist", sim.method = "bray",  p.adjust.m = "BH")




# Second: test comparisons http://www.hiercourse.com/docs/microbial/04_betaDiversity_multTables.html
pairwise.adonis2(dist(t(otu_table(Epi_hindgut2)), method = 'euclidean') ~ Treatment, data = sampledf, strata='Sample.name')




# Third https://github.com/pmartinezarbizu/pairwiseAdonis
Epi_hindgut.rel <- microbiome::transform(Epi_hindgut2, "compositional")

ps_dist_matrix <- phyloseq::distance(Epi_hindgut2, method ="bray")

vegan::adonis2(ps_dist_matrix ~ phyloseq::sample_data(Epi_hindgut.rel)$Treatment)

pairwise.adonis(ps_dist_matrix, phyloseq::sample_data(Epi_hindgut.rel)$Treatment)
...
## **Epibolus hindgut dataset Ordination**

*The Multidimensionnal Scaling (PCoA) or Principal Coordinates Analysis (PCoA) (multivariate analysis) based on Bray-Curtis distance and PCoA ordination for Epibolus hindgut dataset. We can try other estimators* 
```{r Epibolus hindgut multivariate analysis, cache = T}

epi_hindgut.ord <- ordinate(Epi_hindgut2, "PCoA", "bray") 

distanceMethodList

# With PCoA and bray with treatments
epi_hindgut.ord <- ordinate(Epi_hindgut2, "PCoA", "bray") 
phyloseq::plot_ordination(Epi_hindgut2, epi_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Bray")  + geom_point(size= 6)

# With PCoA and chao with treatments
epi_hindgut.ord <- ordinate(Epi_hindgut2, "PCoA", "chao")
phyloseq::plot_ordination(Epi_hindgut2, epi_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Chao")  + geom_point(size= 6)

# With PCoA and betadiver-19 with treatments
epi_hindgut.ord <- ordinate(Epi_hindgut2, "PCoA", "19")
phyloseq::plot_ordination(Epi_hindgut2, epi_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + betadiver-19")  + geom_point(size= 6)

# With PCoA and Jaccard with treatments
epi_hindgut.ord <- ordinate(Epi_hindgut2, "PCoA", "jaccard")
phyloseq::plot_ordination(Epi_hindgut2, epi_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Jaccard")  + geom_point(size= 6)
```
PCoA with bray explained 27 % on Axis 1.
PCoA and Chao explained 77.7 % on Axis 1.
PCoA and Betadiver-19 explained 34.8 % on Axis 1.
PCoA and Jaccard explained 21 % on Axis 1.
To summarie all, the Control and Sterile-fed tend to be ordinated closer together, showing smaller dissimilarity. The two antibiotic treatments are ordinated together and a bit far from the other treatment groups.

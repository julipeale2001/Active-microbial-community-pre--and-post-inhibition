...
*Barplot of G. connexa faecal pellet bacterial copy numbers for all replicates*
```{r plot all replicate faecal copy numbers by Glomeris, cache=TRUE}
# Barplot for all total copy numbers
ggplot(Glo_Faecal, aes(x = Code, y = `Total Copy Numbers of 16S rRNA`, fill = Treatment, group = Code)) + 
...
ggplot(Glo_Faecal, aes(x = Code, y = `16S rRNA Copies/1 ng DNA`, fill = Treatment, group = Code)) + 
  geom_bar(stat = 'identity', position='dodge') +  xlab("Groups") + scale_y_continuous(label=scientific)

# Barplot for all 16S rRNA Copies/mg sample weight
ggplot(Glo_Faecal, aes(x = Code, y = `16S rRNA Copies/mg sample weight`, fill = Treatment, group = Code)) + 
  geom_bar(stat = 'identity', position='dodge') +  xlab("Groups") + scale_y_continuous(label=scientific)

```

*Barplot of G. connexa faecal bacterial mean copy numbers**
```{r plot mean+SE faecal copy numbers by Glomeris, cache=TRUE}
...
#### Anova tests
**Compare G. connexa faecal copy numbers between groups**
```{r faecal copy number aov by Glomersi, cache=T}

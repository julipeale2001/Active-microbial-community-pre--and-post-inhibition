...
## *G. connexa* faecal pellet bacterial 16S copy number analysis
```{r load glomeris faecal copy number data, cache = T}
read_csv("12_Glomeris_Faecal-Pellet_Copy_Numbers.csv",
         trim_ws = TRUE,
...
*Barplot of G. connexa faecal pellet bacterial total 16S rRNA Copies for all replicates*
```{r plot all replicate faecal copy numbers by Epibolus, cache=TRUE}
# Barplot for all Total 16S rRNA Copies
ggplot(Glo_Faecal, aes(x = Code, y = `Total 16S rRNA Copies`, fill = Treatment, group = Code)) + 
  geom_bar(stat = 'identity', position='dodge') +  xlab("Groups") + scale_y_continuous(label=scientific)
```

*Barplot of G. connexa faecal bacterial mean copy numbers**
```{r plot mean+SE faecal copy numbers by Epibolus, cache=TRUE}

# Barplot for Total 16S rRNA Copies
 Glo_Faecal %>%  # the name of the new data frame and the data frame to be summarised
...
#### Anova tests
**Compare G. connexa faecal total 16S rRNA Copies**
...
Glo_Faecal_model1 <- Glo_Faecal %>% 
                     lm(`Total 16S rRNA Copies` ~ Treatment, data = .)

print(Anova(Glo_Faecal_model1, type = "III"))

# checking model assumptions
check_model(Glo_Faecal_model1)

Glo_Faecal_emm_mod1 <- emmeans(Glo_Faecal_model1, as.formula(paste("", "Treatment", sep = " ~ ")), weights = "equal")
      
# Contrasts, pairwise comparisons, tests, and confidence intervals.
Glo_Faecal_emm_mod_cons1 <- contrast(Glo_Faecal_emm_mod1, method = "pairwise", adjust = "BH")
print(Glo_Faecal_emm_mod_cons1)
...
# check the model
Glo_Faecal_model2 <- Glo_Faecal %>% 
                     lm(log_copies ~ Treatment, data = .)
print(Anova(Glo_Faecal_model2, type = "III"))


# checking model assumptions
check_model(Glo_Faecal_model2)

Glo_Faecal_emm_mod2 <- emmeans(Glo_Faecal_model2, as.formula(paste("", "Treatment", sep = " ~ ")), weights = "equal")
      
# Contrasts, pairwise comparisons, tests, and confidence intervals.
Glo_Faecal_emm_mod_cons2 <- contrast(Glo_Faecal_emm_mod2, method = "pairwise", adjust = "BH")
print(Glo_Faecal_emm_mod_cons2)
...
bptest(Glo_Faecal_model1)

# Perform Weighted Least Squares Regression
#define weights to use
wt <- 1 / lm(abs(Glo_Faecal_model1$residuals) ~ Glo_Faecal_model1$fitted.values)$fitted.values^2

#perform weighted least squares regression
wls_model <- lm(`Total 16S rRNA Copies` ~ Treatment, data = Glo_Faecal, weights=wt)
...
# Test for heteroscedasticity, we can perform a Breusch-Pagan test
# perform Breusch-Pagan test
bptest(Glo_Faecal_model2)

# Perform Weighted Least Squares Regression
#define weights to use
wt <- 1 / lm(abs(Glo_Faecal_model2$residuals) ~ Glo_Faecal_model2$fitted.values)$fitted.values^2

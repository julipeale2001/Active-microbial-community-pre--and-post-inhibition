...
## **Glomeris hindgut β-diversity indices**
...
```{r Glomeris hindgut β-diversity indices, cache = T}

# Since, there is no phylogenetic tree for this data set, Bray-Curtis distance will be calculated.
Glo_hindgut.rel <- microbiome::transform(Glo_hindgut2, "compositional")

Glo_hindgut.ord_pcoa_bray <- ordinate(Glo_hindgut.rel, "CAP", "bray", formula =  ~ Treatment) # Produce uncorrelated data

anova(Glo_hindgut.ord_pcoa_bray)
anova(Glo_hindgut.ord_pcoa_bray, by = "axis")

# Scree plot
plot_scree(Glo_hindgut.ord_pcoa_bray) + theme_bw() # Axis 1 and 2 are of interest.
...
Glo_hindgut_beta.ps1 <- phyloseq::plot_ordination(Glo_hindgut.rel,
                            Glo_hindgut.ord_pcoa_bray, 
                            color="Treatment", 
                            label = "Sample.name") + 
  geom_point(size= 8) + 
...
Glo_hindgut_beta.ps1 <- Glo_hindgut_beta.ps1 + theme_bw(base_size = 25) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  geom_line() 
Glo_hindgut_beta.ps1
...
```{r Glomeris hindgut Permanova for beta diversity, cache = T}
set.seed(10000)
# Calculate bray curtis distance matrix
erie_bray <- phyloseq::distance(Glo_hindgut2, method = "bray", formula =  ~ Treatment)

# make a data frame from the sample_data
sampledf <- data.frame(sample_data(Glo_hindgut2))

# Adonis test
adonis2(erie_bray ~ Treatment, data = sampledf, permutations = 999)

    
# Homogeneity of dispersion test
beta <- betadisper(erie_bray, sampledf$Treatment)
permutest(beta)
plot(beta,
...
pairwise_adonis(x = t(otu_table(Glo_hindgut2)), factor = get_variable(Glo_hindgut2, "Treatment"), sim.function = "vegdist", sim.method = "bray",  p.adjust.m = "BH")

```
This output tells us that our adonis test is insignificant so we can accept the null hypothesis that our the treatment samples have almost same microbial communities. Additionally, our betadisper results are insignificant, meaning we accept the null hypothesis that our treatments have the same dispersions. This means we can be more confident that our adonis result is a real result due to the similarities in group dispersions.


## **Glomeris hindgut dataset Ordination**

*The Multidimensionnal Scaling (PCoA) or Principal Coordinates Analysis (PCoA) (multivariate analysis) based on Bray-Curtis distance and PCoA ordination for Glomeris hindgut dataset. We can try other estimators* 
```{r Glomeris hindgut multivariate analysis, cache = T}

Glo_hindgut.ord <- ordinate(Glo_hindgut2, "PCoA", "bray") 

distanceMethodList

# With PCoA and bray with treatments
Glo_hindgut.ord <- ordinate(Glo_hindgut2, "PCoA", "bray") 
phyloseq::plot_ordination(Glo_hindgut2, Glo_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Bray")  + geom_point(size= 6)

# With PCoA and chao with treatments
Glo_hindgut.ord <- ordinate(Glo_hindgut2, "PCoA", "chao")
phyloseq::plot_ordination(Glo_hindgut2, Glo_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Chao")  + geom_point(size= 6)

# With PCoA and betadiver-19 with treatments
Glo_hindgut.ord <- ordinate(Glo_hindgut2, "PCoA", "19")
phyloseq::plot_ordination(Glo_hindgut2, Glo_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + betadiver-19")  + geom_point(size= 6)

# With PCoA and Jaccard with treatments
Glo_hindgut.ord <- ordinate(Glo_hindgut2, "PCoA", "jaccard")
phyloseq::plot_ordination(Glo_hindgut2, Glo_hindgut.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Jaccard")  + geom_point(size= 6)
```
There is no clear ordination.

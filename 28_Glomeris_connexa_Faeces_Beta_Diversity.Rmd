...
## **Glomeris faecal β-diversity indices**
...
```{r Glomeris faecal β-diversity indices, cache = T}
Glo_faecal.rel <- microbiome::transform(Glo_faecal2, "compositional")

Glo_faecal.ord_pcoa_bray <- ordinate(Glo_faecal.rel, "CAP", "bray", formula =  ~ Treatment)

anova(Glo_faecal.ord_pcoa_bray)
anova(Glo_faecal.ord_pcoa_bray, by = "axis")

# Scree plot
plot_scree(Glo_faecal.ord_pcoa_bray) + theme_bw() # Axis 1 and 2 are of interest.

Glo_faecal_beta.ps1 <- phyloseq::plot_ordination(Glo_faecal.rel, 
                            Glo_faecal.ord_pcoa_bray, 
                            color="Treatment", 
                            label = "Sample.name") + 
...
Glo_faecal_beta.ps1 <- Glo_faecal_beta.ps1 + theme_bw(base_size = 25) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  geom_line() 
Glo_faecal_beta.ps1
ggsave("Glo_Faeces_CAP.svg",width = 30, height = 20, units = "cm")
...
```{r Glomeris faecal Permanova for beta diversity, cache = T}
set.seed(10000)
# Calculate bray curtis distance matrix
Glo_faecal.erie_bray <- phyloseq::distance(Glo_faecal2, method = "bray")

# make a data frame from the sample_data
Glo_faecal.sampledf <- data.frame(sample_data(Glo_faecal2))

# Adonis test
adonis2(Glo_faecal.erie_bray ~ Treatment, data = Glo_faecal.sampledf)
    
# Homogeneity of dispersion test
Glo_faecal.beta <- betadisper(Glo_faecal.erie_bray, Glo_faecal.sampledf$Treatment)
Glo_faecal.beta

plot(Glo_faecal.beta,
     main="Permanova for Beta Diversity", col = c("red", "blue", "green", "orange"))

boxplot(Glo_faecal.beta, xlab = "", las = 2, cex.axis = 0.8)

anova(Glo_faecal.beta)
permutest(Glo_faecal.beta)
TukeyHSD(Glo_faecal.beta)


# Define pairwise comparisons
pairwise_adonis(x = t(otu_table(Glo_faecal2)), factor = get_variable(Glo_faecal2, "Treatment"), sim.function = "vegdist", sim.method = "bray",  p.adjust.m = "BH")
```
This output tells us that our adonis test is insignificant so we can accept the null hypothesis that our the treatment samples have same microbial communities. Additionally, our betadisper results are insignificant, meaning we accept the null hypothesis that our treatments have the same dispersions. This means we can be more confident that our adonis result is a real result due to the differences in group dispersions.



## *Glomeris faecal Ordination*

*The Multidimensionnal Scaling (PCoA) or Principal Coordinates Analysis (PCoA) (multivariate analysis) based on Bray-Curtis distance and PCoA ordination for Glomeris faecal. We used other estimators*
```{r Glomeris faecal multivariate analysis, cache = T}

Glo_faecal.ord <- ordinate(Glo_faecal2, "PCoA", "bray")

# With PCoA and bray with treatments
Glo_faecal.ord <- ordinate(Glo_faecal2, "PCoA", "bray")
phyloseq::plot_ordination(Glo_faecal2, Glo_faecal.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Bray") + geom_point(size= 6)

# With PCoA and chao with treatments
Glo_faecal.ord <- ordinate(Glo_faecal2, "PCoA", "chao")
phyloseq::plot_ordination(Glo_faecal2, Glo_faecal.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Chao")  + geom_point(size= 6)

# With PCoA and betadiver-19 with treatments
Glo_faecal.ord <- ordinate(Glo_faecal2, "PCoA", "19")
phyloseq::plot_ordination(Glo_faecal2, Glo_faecal.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + betadiver-19")  + geom_point(size= 6)

# With PCoA and Jaccard with treatments
Glo_faecal.ord <- ordinate(Glo_faecal2, "PCoA", "jaccard")
phyloseq::plot_ordination(Glo_faecal2, Glo_faecal.ord, color = "Treatment") + theme_bw() + ggtitle("PCoA + Jaccard")  + geom_point(size= 6)
```
No clear ordination.

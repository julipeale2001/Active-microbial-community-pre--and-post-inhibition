...
plot_DESeq <- function(DESeq_results, ps_obj, sig_level = alpha_thresh, LFC = LFC_thresh, rank = "Phylum", rare_thresh = 0.0045, ASV_labels = FALSE, Y_val = "log2FoldChange", plot_title = "") {
  require(ggplot2)
  require(ggrepel)
  require(ggtext)
...
plot_ggtree <- function(ps_obj, 
  ...
  p_tree <- ggtree(ps_obj,
    ...
    theme_tree(legend.title = element_markdown(size = f_size - 4),
               ...
               legend.text = element_text(size = f_size - 6, face = "italic"),
               legend.box = "vertical", 
               legend.margin = margin(),
               plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"),
               plot.title = element_text(size = f_size - 2, face = "italic")) +
...
get_variable(Ps_obj_abs) %>% 
  ...
  qPCR_df
plot_SIP_fractions(qPCR_df, colour = "Rep_label", xticks = seq(1.71, 1.85, by = 0.03)) 
```

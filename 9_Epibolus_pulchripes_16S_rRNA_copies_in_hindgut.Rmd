...
```{r load hindgut copy number data by Epibolus, cache = T}
read_csv("9_Epibolus_Hindgut_Copy_Numbers.csv",
         trim_ws = TRUE,
...
```{r plot all hindgut copy numbers by Epibolus, cache=TRUE}
# Barplot for all total copy numbers
ggplot(Epi_Hindgut, aes(x = Code, y = `Total Copy Numbers of 16S rRNA`, fill = Treatment, group = Code)) + 
...
ggplot(Epi_Hindgut, aes(x = Code, y = `16S rRNA Copies/1 ng DNA`, fill = Treatment, group = Code)) + 
  geom_bar(stat = 'identity', position='dodge') +  xlab("Groups") + scale_y_continuous(label=scientific)

# Barplot for all 16S rRNA Copies/mg sample weight
ggplot(Epi_Hindgut, aes(x = Code, y = `16S rRNA Copies/mg sample weight`, fill = Treatment, group = Code)) + 
  geom_bar(stat = 'identity', position='dodge') +  xlab("Groups") + scale_y_continuous(label=scientific)

```

*Barplot of E. pulchripes hindgut bacterial mean copy numbers**
```{r plot mean+SE hindgut copy numbers by Epibolus, cache = TRUE}
# Barplot for mean total copy numbers
 Epi_Hindgut %>%  # the name of the new data frame and the data frame to be summarised
...
```{r  hindgut copy number aov by Epibolus, cache=T}
...
```{r  hindgut copy number post-hoc by Epibolus, cache=T}

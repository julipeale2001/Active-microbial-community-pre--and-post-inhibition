...
## **Alpha diversity for Globolus hindgut dataset**

*Diversity indices in Globolus hindgut dataset*

*Rarefy without replacement and plot richness*
```{r Globolus hindgut Rarefy without replacement, cache = T}


# Plot richness for all samples
theme_set(theme_bw())
Glo_hindgut_plot_richness <- plot_richness(Glo_hindgut, x = 'Sample.name', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
...
Glo_hindgut_plot_richness$data$Treatment = factor(Glo_hindgut_plot_richness$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_hindgut_plot_richness


# Plot richness for all treatments
theme_set(theme_bw())
Glo_hindgut_plot_richness1 <- plot_richness(Glo_hindgut, x = 'Treatment', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
  theme(axis.text.x = element_text(
    angle = 90,
...
Glo_hindgut_plot_richness1$data$Treatment = factor(Glo_hindgut_plot_richness1$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
Glo_hindgut_plot_richness1

ggsave("Glo_hindgut_richness_its.png", device = "png", path = "Richness_plot")
...
```{r Globolus hindgut measure Richness, Shannon, for the phyloseq object, cache = T}
Glo_hindgut_alpha.its <- estimate_richness(Glo_hindgut, measures=c("Observed", "Chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "Fisher"))
Glo_hindgut_alpha.its


# Add SampleID column to the alpha dataframe
Glo_hindgut_alpha.its$Sample <- sample_names(Glo_hindgut)

# Merge alpha and Globolus hindgut dataset into a single dataframe
Glo_hindgut_metad_alpha_its <- merge(psmelt(Glo_hindgut), Glo_hindgut_alpha.its)

# Write it into a csv file
write.csv(Glo_hindgut_metad_alpha_its, file.path("Glo_hindgut_alpha_its.csv")) # writes out a .csv file that you can open in excel
```
Let's go ahead and write only metrics with mean and sd in table
```{r Globolus hindgut summarize our models, cache = T}
...
# Mean for treatment
Glo_hindgut_alphmean.its = aggregate(Glo_hindgut_metad_alpha_its[,c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")],list(Glo_hindgut_metad_alpha_its$Sample.name, Glo_hindgut_metad_alpha_its$Treatment), mean)

# SD for treatment
Glo_hindgut_alphsd.its = aggregate(Glo_hindgut_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Glo_hindgut_metad_alpha_its$Sample.name, Glo_hindgut_metad_alpha_its$Treatment), sd)

# SE for treatment
se <- function(x) sd(x) / sqrt(length(x))
Glo_hindgut_alphse.its = aggregate(Glo_hindgut_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Glo_hindgut_metad_alpha_its$Sample.name, Glo_hindgut_metad_alpha_its$Treatment), se)

# Rename the columns
colnames(Glo_hindgut_alphmean.its) <- c("Sample.name", "Treatment",  "Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")
colnames(Glo_hindgut_alphsd.its) <- c("Sample.name", "Treatment", "Observed.sd", "Chao1.sd", "se.chao1.sd", "Shannon.sd", "Simpson.sd", "InvSimpson.sd", "ACE.sd", "se.ACE.sd", "Fisher.sd")
colnames(Glo_hindgut_alphse.its) <- c("Sample.name", "Treatment", "Observed.se", "Chao1.se", "se.chao1.se", "Shannon.se", "Simpson.se", "InvSimpson.se", "ACE.se", "se.ACE.se", "Fisher.se")

    
# Merge and write into a csv file
Glo_hindgut.mean.sd.se <- Reduce(merge, list(Glo_hindgut_alphmean.its, Glo_hindgut_alphsd.its, Glo_hindgut_alphse.its))
options(digits=2)

write.csv(Glo_hindgut.mean.sd.se, file.path("Glo_hindgut_alpha_mean_sd_se.csv"))
```
...
data_melted <- reshape2::melt(Glo_hindgut.mean.sd.se, id.vars = c("Sample.name", "Treatment")) %>%
...
p_values_list_expanded %>%
  mutate(Treatment = factor(Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))) %>%
...
p_Glo_hindgut <- ggplot(data_melted, aes(x = Treatment, y = value, fill = Treatment)) +
  geom_violin() +
...
data_melted$Treatment <- factor(data_melted$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
...
# Summarize data
Glo_data_summarized <- data_melted %>%
  group_by(Treatment, variable) %>%
...
# Compute Kruskal-Wallis p-values for each Treatment and variable combination
p_values_list <- Glo_data_summarized %>%
  group_by(variable) %>%
  do(p_value = compute_kruskal_pvalue(.)) %>%
  ungroup()
...
change <- c("Antibiotics(5X)" = "Antibiotics (5X)", 
...
data_frames$Treatment <- factor(data_frames$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (5X)", "Antibiotics (2X)"))
...
ggsave("Glo_hindgut_alpha_diversity.svg",width = 20, height = 35, units = "cm")
```

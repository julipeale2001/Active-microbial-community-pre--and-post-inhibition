...
subtitle: "Epibolus Faecal Pellet"
author: "Julius Nweze"
date: "`r Sys.Date()`"
output:
...
## **Exploring biodiversity: visualization in Epibolus faecal dataset**

*Normalize number of reads in each Epibolus faecal sample using median sequencing depth.*
```{r Epibolus faecal normalization of reads, cache = T}
total = median(sample_sums(Epi_faecal2))
standf = function(x, t=total) round(t * (x / sum(x)))
Epi_faecal2 = transform_sample_counts(Epi_faecal2, standf)
```



*Epibolus faecal dataset bar graphs*

*Making the bar graph nicer by removing ASVs boundaries for Epibolus faecal dataset. This is done by adding ggplot2 modifier.*
```{r adding ggplot2 modifier Epibolus faecal, cache = T}
 plot_bar(Epi_faecal2, fill = "Phylum") + 
...
*Regrouping together according to Epibolus faecal's treatments*
```{r regroup together according to samples types Epibolus faecal, cache = T}
# Merge by treatment
epi_faecal_fraction <- merge_samples(Epi_faecal2, "Treatment")
# Repair the merged values associated with each surface after merge.
sample_data(epi_faecal_fraction)$Treatment <- levels(sample_data(epi_faecal_fraction)$Treatment)

# Flip

sample_data(epi_faecal_fraction)$Treatment <- sample_names(epi_faecal_fraction)
ps2.epi_faecal_fraction <- transform_sample_counts(epi_faecal_fraction,  
                                    function(x) x/sum(x) * 100)
p <- plot_bar(ps2.epi_faecal_fraction, fill = "Phylum") + 
...
## **Heatmaps for Epibolus faecal dataset**

*Taking ASVs that represent at least 5% of reads in at least one  Epibolus faecal sample. Remember we normalized all the samples to median number of reads (total). We are left with only taxa which makes the reading much more easy.*
```{r filtering taxa in Epibolus faecal, cache = T}
# Let us use 5% here
epi_faecal_abund <- filter_taxa(Epi_faecal2, function(x) sum(x > total*0.05) > 0, TRUE)
epi_faecal_abund
```



*New heatmap for Epibolus faecal dataset*
```{r Epibolus faecal tsv table and heatmap, cache = T}
plot_heatmap(epi_faecal_abund, method = "PCoA", distance = "bray")
```


*Using different distances and different multivaraite methods (Jaccard distance and NPCoA and label ASVs with Class, order by Class) in the Epibolus faecal dataset. We can also change the Palette (the default palette is a bit ugly…).*
```{r Epibolus faecal heatmap with different distances and different multivaraite methods, cache = T}
plot_heatmap(epi_faecal_abund, method = "PCoA", distance = "jaccard",
...
**Phylum to Genus Distributions in the Epibolus faecal dataset using the most abundant ASVs** 

*Phylum distribution in Epibolus faecal dataset using top phyla*
```{r most abundant ASVs for phylum distribution Epibolus faecal, cache = T}
# Let us use 0.5%
Epi_faecal_abundant <- filter_taxa(Epi_faecal2, function(x) sum(x > total*0.005) > 0, TRUE)
Epi_faecal_abundant

# Creating 100% abundance stacked bar plots with all the phylum

Epi_faecal_a <- ggplot(data = psmelt(Epi_faecal_abundant), mapping = aes_string(x = "Treatment",y = "Abundance")) + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="fill")
Epi_faecal_a$data$Treatment <- factor(Epi_faecal_a$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
Epi_faecal_a
```
...
*Agglomerate taxa*
```{r Agglomerate taxa most abundant ASVs for phylum distribution Epibolus faecal, cache = T}
library(plyr); library(dplyr)
Epi_faecal_GMPR_rel <- transform_sample_counts(Epi_faecal1, function(x) x / sum(x) )
...
Epi_faecal_100 <-
  prune_taxa(names(sort(taxa_sums(Epi_faecal_GMPR_rel), TRUE)[1:100]), Epi_faecal_GMPR_rel)
plot_heatmap(
  Epi_faecal_100,
  method = NULL,
  distance = NULL,
...
```{r Checking agglo taxa most abundant ASVs for phylum distribution Epibolus faecal, cache = T}

Epi_faecal_glom <- tax_glom(Epi_faecal1, 
                             "Phylum", 
                             NArm = TRUE)
Epi_faecal_glom_rel <- transform_sample_counts(Epi_faecal_glom, function(x) x / sum(x)) 
Epi_faecal_glom_rel_DF <- psmelt(Epi_faecal_glom_rel)
Epi_faecal_glom_rel_DF$Phylum %<>% as.character()

# group dataframe by Phylum, calculate median rel. abundance
detach(package:plyr)    
library(dplyr) # OR df1 <- ddply(Epi_faecal_glom_rel_DF, .(Phylum), summarise, median = median(Abundance))
Epi_faecal_glom_rel_DF %>%
  group_by(Phylum) %>%
  summarise(median = median(Abundance)) ->
  medians

# find Phyla whose rel. abund. is less than 0.5%
Other_phyla <- medians[medians$median <= 0.005, ]$Phylum

# change their name to "Rares"
Epi_faecal_glom_rel_DF[Epi_faecal_glom_rel_DF$Phylum %in% Other_phyla, ]$Phylum <- 'Rares'
# re-group
Epi_faecal_glom_rel_DF %>%
  group_by(Sample, Sample.name, Treatment, Phylum, Sample.type) %>%
  summarise(Abundance = sum(Abundance)) ->
  Epi_faecal_glom_rel_DF_2plot

# ab.taxonomy$Freq <- sqrt(ab.taxonomy$Freq)
Epi_faecal_glom_rel_DF_2plot$Phylum %<>% sub("unclassified", "Unclassified", .)
Epi_faecal_glom_rel_DF_2plot$Phylum %<>% sub("uncultured", "Unclassified", .)
...
Epi_faecal_glom_rel_DF_2plot$Phylum %<>% 
  factor(., levels = Taxa_rank$Phylum) %>% 
  fct_relevel(., "Rares", after = Inf)
  
p_taxa_box <-
  ggplot(Epi_faecal_glom_rel_DF_2plot, aes(x = Phylum, y = (Abundance*100), fill = Treatment)) + scale_y_continuous(limits=c(0,100)) +
...
Epi_faecal_d <- ggplot(Epi_faecal_glom_rel_DF_2plot, mapping = aes(x=Phylum, y=Abundance, fill = Treatment)) + geom_boxplot() +  scale_color_hue(l=40, c=35) + geom_point(aes(colour = Treatment), position = position_jitterdodge(dodge.width = 1), alpha = 1 / 2, stroke = 0, size = 2) + scale_y_continuous(limits=c(0,1)) +
  theme(text = element_text(size = 18),  
axis.text = element_text(size = 18))+coord_flip()
Epi_faecal_d$data$Treatment <- factor(Epi_faecal_d$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
print(Epi_faecal_d)
...
```{r Creating 100% abundance stacked bar plots for most abundant ASVs for phylum distribution Epibolus faecal, cache = T}

Epi_faecal_b <- ggplot(data = Epi_faecal_glom_rel_DF_2plot, mapping = aes_string(x = "Treatment", y = "Abundance")) + geom_bar(aes(fill  = Phylum), stat = "identity", position= "fill") + ylab("Relative Abundance") +
theme(text = element_text(size = 18),  
axis.text = element_text(size = 18))+coord_flip() + scale_fill_brewer("Phylum", palette = "Dark2") 
Epi_faecal_b$data$Treatment <- factor(Epi_faecal_b$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
#Epi_faecal_b$data$Phylum <- factor(Epi_faecal_b$data$Phylum, levels = c("Bacteroidota", "Proteobacteria", "Firmicutes", "Desulfobacterota", "Verrucomicrobiota", "Planctomycetota", "Thermoplasmatota", "Halobacterota", "Acidobacteriota", "Rares"))

Epi_faecal_b
```
Proteobacteria are the most abundant Phylum in all the treatments (highest in the Antibiotics 1X). In the Control and Sterile-fed, Actinobacteriota and Bacteriodota are respectively the second most abundant groups. The second predominant groups in the Antibiotics (2X) and Antibiotics (1X) are Actinobacteriota and Bacteriodota respectively. 


*Genus distribution in Epibolus faecal dataset using ASVs that represent at least 16% of reads in at least one sample*
```{r most abundant ASVs for Genus distribution Epibolus faecal, cache = T}
# Let us use 0.5%
Epi_faecal_abundant <- filter_taxa(Epi_faecal2, function(x) sum(x > total*0.005) > 0, TRUE)
Epi_faecal_abundant
```
...
*Summarizing relative Genus abundance in Epibolus dataset*

Let’s look at the agglomerated Genus

```{r Checking agglo taxa most abundant ASVs for Genus distribution Epibolus faecal, cache = T}
...
Epi_faecal_glom_rel <- transform_sample_counts(Epi_faecal_glom, function(x) x / sum(x)) 
Epi_faecal_glom_rel_DF <- psmelt(Epi_faecal_glom_rel)
Epi_faecal_glom_rel_DF$Genus %<>% as.character()

# group dataframe by Genus, calculate median rel. abundance
#detach(package:plyr)    
#library(dplyr) # OR df1 <- ddply(Epi_faecal_glom_rel_DF, .(Genus), summarise, median = median(Abundance))
Epi_faecal_glom_rel_DF %>%
  group_by(Genus) %>%
  summarise(median = median(Abundance)) ->
  medians
...
# change their name to "Rares"
Epi_faecal_glom_rel_DF[Epi_faecal_glom_rel_DF$Genus %in% Other_phyla, ]$Genus <- 'Rares'
# re-group
Epi_faecal_glom_rel_DF %>%
...
# ab.taxonomy$Freq <- sqrt(ab.taxonomy$Freq)
Epi_faecal_glom_rel_DF_2plot$Genus %<>% sub("unclassified", "Unclassified", .)
Epi_faecal_glom_rel_DF_2plot$Genus %<>% sub("uncultured", "Unclassified", .)
...
Epi_faecal_glom_rel_DF_2plot %>% 
  ...
  summarise(sum.Taxa = sum(Abundance)) %>% 
  arrange(desc(sum.Taxa)) -> Taxa_rank
Epi_faecal_glom_rel_DF_2plot$Genus %<>% 
  factor(., levels = Taxa_rank$Genus) %>% 
  fct_relevel(., "Rares", after = Inf)
  
p_taxa_box <-
  ggplot(Epi_faecal_glom_rel_DF_2plot, aes(x = Genus, y = (Abundance*100), fill = Treatment)) + scale_y_continuous(limits=c(0,100)) +
...
## Using it to make joined Boxplot
Epi_faecal_d <- ggplot(Epi_faecal_glom_rel_DF_2plot, mapping = aes(x=Genus, y=Abundance, fill = Treatment)) + geom_boxplot() +  scale_color_hue(l=40, c=35) + geom_point(aes(colour = Treatment), position = position_jitterdodge(dodge.width = 1), alpha = 1 / 2, stroke = 0, size = 2) + scale_y_continuous(limits=c(0,1)) +
  theme(axis.text.x = element_text(
    angle = 45,
    vjust = 0.9,
    hjust = 0.9
...
```{r Creating 100% abundance stacked bar plots for most abundant ASVs for Genus distribution Epibolus faecal, cache = T}
grid.colour = c(Citrobacter  = "#006400", Desulfovibrio  = "#A52A2A", Bacteroides  = "#008080", Alistipes  = "#8A2BE2", Candidatus_Symbiothrix  = "#808000", Candidatus_Methanoplasma = "#00FF00", Pseudomonas = "#00FFFF", Dysgonomonas  = "#FF00FF", Achromobacter  = "#800000", Thauera = "#FF6347", Klebsiella = "#FFFF00", Novosphingobium = "#0000FF", Mycobacterium  = "#808080", `Burkholderia-Caballeronia-Paraburkholderia` = "#87CEFA",  Rares = "#C0C0C0")
...
Most Abundant:
Control -Bacteroides
Sterile-fed -Bacteroides followed by Paraburkholderia
Antibiotics (2X) -Citrobacter followed by Novosphingobium
Antibiotics (1X) -Novosphingobium followed by Citrobacter

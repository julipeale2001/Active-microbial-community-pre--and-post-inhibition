...
Epi_faecal_b <- ggplot(data = Epi_faecal_glom_rel_DF_2plot, mapping = aes_string(x = "Treatment", y = "Abundance")) + geom_bar(aes(color = Genus, fill  = Genus), stat = "identity", position= "fill") + ylab("Relative Abundance")
Epi_faecal_b$data$Treatment <- factor(Epi_faecal_b$data$Treatment, levels = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
Epi_faecal_b
```
Most Abundant:

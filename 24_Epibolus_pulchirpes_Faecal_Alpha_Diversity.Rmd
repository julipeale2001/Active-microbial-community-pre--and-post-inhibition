...
##**Alpha diversity for Epibolus faecal dataset**

*Diversity indices in Epibolus faecal dataset*

*Rarefy without replacement and plot richness*
```{r Epibolus faecal Rarefy without replacement, cache = T}
# Plot richness using rarefied dataset for all samples
theme_set(theme_bw())
Epi_faecal_plot_richness <- plot_richness(Epi_faecal, x = 'Sample.name', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
...
Epi_faecal_plot_richness$data$Treatment = factor(Epi_faecal_plot_richness$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
Epi_faecal_plot_richness

# Plot richness using rarefied dataset for all treatments
theme_set(theme_bw())
Epi_faecal_plot_richness1 <- plot_richness(Epi_faecal, x = 'Treatment', measures = c('Chao1', 'Shannon', 'observed', "Simpson", "InvSimpson", "ACE", "Fisher"), color = 'Treatment') +
  ggtitle("a)") +
  xlab(" ") +
  theme(axis.text.x = element_text(
    angle = 45,
...
Epi_faecal_plot_richness1$data$Treatment = factor(Epi_faecal_plot_richness1$data$Treatment, level = c("Control", "Sterile feed", "Antibiotics (2X)", "Antibiotics (1X)"))
Epi_faecal_plot_richness1

ggsave("Epi_faecal_richness_its.png", device = "png", path = "Richness_plot")
...
```{r Epibolus faecal measure Richness, Shannon, for the phyloseq object, cache = T}
Epi_faecal_alpha.its <- estimate_richness(Epi_faecal, measures=c("Observed", "Chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "Fisher"))
Epi_faecal_alpha.its


# Add SampleID column to the alpha dataframe
Epi_faecal_alpha.its$Sample <- sample_names(Epi_faecal)

# Merge alpha and Epibolus faecal dataset into a single dataframe
Epi_faecal_metad_alpha_its <- merge(psmelt(Epi_faecal), Epi_faecal_alpha.its)

# Write it into a csv file
write.csv(Epi_faecal_metad_alpha_its, file.path("Epi_faecal_alpha_its.csv")) # writes out a .csv file that you can open in excel
```
Let's go ahead and write only metrics with mean and sd in table
```{r Epibolus faecal summarize our models, cache = T}
...
# Mean for treatment
Epi_faecal_alphmean.its = aggregate(Epi_faecal_metad_alpha_its[,c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")],list(Epi_faecal_metad_alpha_its$Sample.name, Epi_faecal_metad_alpha_its$Treatment), mean)

# SD for treatment
Epi_faecal_alphsd.its = aggregate(Epi_faecal_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Epi_faecal_metad_alpha_its$Sample.name, Epi_faecal_metad_alpha_its$Treatment), sd)

# SE for treatment
se <- function(x) sd(x) / sqrt(length(x))
Epi_faecal_alphse.its = aggregate(Epi_faecal_metad_alpha_its[, c("Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")], list(Epi_faecal_metad_alpha_its$Sample.name, Epi_faecal_metad_alpha_its$Treatment), se)

# Rename the columns
colnames(Epi_faecal_alphmean.its) <- c("Sample.name", "Treatment", "Observed", "Chao1", "se.chao1", "Shannon", "Simpson", "InvSimpson", "ACE", "se.ACE", "Fisher")
colnames(Epi_faecal_alphsd.its) <- c("Sample.name", "Treatment", "Observed.sd", "Chao1.sd", "se.chao1.sd", "Shannon.sd", "Simpson.sd", "InvSimpson.sd", "ACE.sd", "se.ACE.sd", "Fisher.sd")
colnames(Epi_faecal_alphse.its) <- c("Sample.name", "Treatment", "Observed.se", "Chao1.se", "se.chao1.se", "Shannon.se", "Simpson.se", "InvSimpson.se", "ACE.se", "se.ACE.se", "Fisher.se")

    
# Merge and write into a csv file
Epi_faecal.mean.sd.se <- Reduce(merge, list(Epi_faecal_alphmean.its, Epi_faecal_alphsd.its, Epi_faecal_alphse.its))
options(digits=2)

write.csv(Epi_faecal.mean.sd.se, file.path("Epi_faecal_alpha_mean_sd_se.csv"))
```
...
data_melted <- reshape2::melt(Epi_faecal.mean.sd.se, id.vars = c("Sample.name", "Treatment")) %>%
...
p_Epi_faecal <- ggplot(data_melted, aes(x = Treatment, y = value, fill = Treatment)) +
  geom_violin() +
...
ggsave("Epi_faecal_alpha_diversity.svg",width = 20, height = 35, units = "cm")
```
